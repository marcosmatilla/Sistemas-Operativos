...
int Clock_GetTimeExamen(); //Examen-simulacro-2020
int Clock_GetTime();
...
int response_time = 0; // -> número de tics de reloj que han transcurrido desde que el proceso ha sido creado y la primera vez que el proceso pasa a ejecutarse.
int return_time = 0;  // ->número de tics de reloj que han transcurrido desde que el proceso ha sido creado y el proceso termina.
int waiting_time = 0; // ->número de tics que el proceso pasa en el estado listo para ejecución. Recuerda que el proceso puede pasar varias veces durante su vida por este estado.
...
void OperatingSystem_Dispatch(int PID) {
	...
	response_time = Clock_GetTimeExamen();
...
void OperatingSystem_TerminateProcess() {
	...
	int selectedProcess;
	//OperatingSystem_ShowTime(SYSPROC);
	//ComputerSystem_DebugMessage(550,SYSPROC,"return time",executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName, return_time);
	...
	return_time=Clock_GetTimeExamen();
	...
	OperatingSystem_ShowTime(SYSPROC);
	ComputerSystem_DebugMessage(550,SYSPROC,"response time",executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName, response_time);

	OperatingSystem_ShowTime(SYSPROC);
	ComputerSystem_DebugMessage(550,SYSPROC,"return time",executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName, return_time);

	OperatingSystem_ShowTime(SYSPROC);
	ComputerSystem_DebugMessage(550,SYSPROC,"waiting time",executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName, waiting_time);
...
void OperatingSystem_HandleClockInterrupt(){ 
	...
	waiting_time = Clock_GetTimeExamen();
