...
void OperatingSystem_HandleSystemCall() {
	...
	int booleanFirst = 0; //Variable booleana
	...
	switch (systemCallID) {
		...
		case SYSCALL_END:
			...
			OperatingSystem_PrintStatus();
			break;
		//Impedir que un proceso ceda el control del procesador, con exito, mas de una vez. Mensaje número 130.
		case SYSCALL_YIELD: //Examen Mayo 2020 
			if(booleanFirst == 1){ //Si entra es porque ha cambiado el estado de la variable y ya no seria el primer proceso
				...
				if(numberOfReadyToRunProcesses[queueID]>0){
					...
					if(process != NOPROCESS){
						if(processTable[executingProcessID].priority == processTable[process].priority){
							...
							OperatingSystem_PrintStatus();
							booleanFirst = 1; //Una vez pasa por aqui la primera vez cambia el valor de la variable para que el else no se vuelva a ejecutar
						}
					}
				}
			}
